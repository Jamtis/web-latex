// hotpatch fetch to absolute urls
const base_url = 'https://foc.ethz.ch/people/nicholasbrandt/web-latex/src/web/swiftlatex/';
const _fetch = fetch;
globalThis.fetch = (url, ...args) => {
    return _fetch(base_url + url, ...args);
};
...
function writeFileRoutine(filename, content) {
    try {
        const parent_path = PATH.normalize('/' + WORKROOT + '/' + filename).match(/^(.*)\/.+$/)[1];
        FS.mkdirTree(parent_path);
